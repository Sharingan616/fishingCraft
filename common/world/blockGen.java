		...
		for (int i = 0; i <= world.getActualHeight(); i++)
		{
			for (int j = 0; j < boundry; j++)
			{
				for (int u = 0; u < boundry; u++)
				{
					...
					//Bee Hive Spawning
					if(currentBiome == BiomeGenBase.forest || currentBiome == BiomeGenBase.birchForest && Math.random() > 0.5)
					{
						if(blockIdAbove == Block.getIdFromBlock(Blocks.leaves) && Block.getIdFromBlock(world.getBlock(xCoord+1, i, zCoord)) == Block.getIdFromBlock(Blocks.log) && Block.getIdFromBlock(world.getBlock(xCoord-1, i, zCoord)) == Block.getIdFromBlock(Blocks.leaves) && blockIdBelow == Block.getIdFromBlock(Blocks.air))
						{
